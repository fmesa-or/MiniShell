...
/*   Updated: 2025/05/01 13:09:55 by fmesa-or         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ms_builts(t_token *token, t_data *data, t_token *token_prev)
{
	...
	ret = 0;
	if ((ft_strncmp(token->argv[0], "cd", ft_strlen(token->argv[0])) == 0) && (token[1].type == NONE && token_prev->type == NONE))
		...
		ret = bi_echo(token);
	else if ((ft_strncmp(token->argv[0], "export", ft_strlen(token->argv[0])) == 0) && (token[1].type == NONE && token_prev->type == NONE))
		ret = bi_export(data->exported_list, token->argv);
	else if ((ft_strncmp(token->argv[0], "unset", ft_strlen(token->argv[0])) == 0) && (token[1].type == NONE && token_prev->type == NONE))
	...
	else if ((ft_strncmp(token->argv[0], "exit", ft_strlen(token->argv[0])) == 0) && (token[1].type == NONE && token_prev->type == NONE))
